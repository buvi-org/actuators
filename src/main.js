...
function selectPart(id, refreshTree = true) {
  ...
  const sources = sourceIds
    ...
    .join("");
  $("component-detail").innerHTML =
    `<h2 class="detail-title">${escape(node.name)}</h2><span class="pill ${study || !items.length ? "amber" : ""}">${escape(node.status || "Assembly group")}</span>${["M02", "EM03"].includes(row?.id) ? `<p class="open-note">Known collision: the rotor end bell intersects rear housing NAUO4 in the assembled model. An axial-layout correction is pending; hiding parts or changing explosion offsets does not resolve it.</p>` : ""}${row?.id === "M01" ? `<p class="open-note">Primeform R1: extended bosses intersect the provisional rotor yoke. Clearance redesign required. Gold bodies are added support material; hide them to compare the original housing.</p><a href="${base}design/main-housing-supported.step" download>Modified housing STEP ↓</a>` : ""}${["G02", "G04", "G05"].includes(row?.id) ? `<p><a href="${base}design/provisional-${{ G02: "sun", G04: "planet", G05: "ring" }[row.id]}-gear.step" download>Single-solid gear STEP ↓</a><br><a href="${base}design/provisional-gear-train.step" download>Gear assembly STEP ↓</a></p>` : ""}<div class="inspector-actions"><button id="isolate-part" ${items.length ? "" : "disabled"}>${isolated === id ? "Exit isolate" : "Isolate"}</button><button id="hide-part" ${items.length ? "" : "disabled"}>${items.some((m) => m.visible) ? "Hide" : "Show"}</button><button id="focus-part" ${items.length ? "" : "disabled"}>Focus</button></div><dl class="detail-grid">${detail}</dl>${row?.unresolved ? `<h3 class="detail-heading">UNRESOLVED DETAILS</h3><p class="open-note">${escape(row.unresolved)}</p>` : ""}<div class="detail-sources">${sources}${node.electronic ? `<a href="${node.electronic.source}" target="_blank" rel="noreferrer">Driver STEP source ↗</a>` : ""}</div>`;
...
async function init3D() {
  ...
  const [planetSolid, ringSolid] = await Promise.all([
